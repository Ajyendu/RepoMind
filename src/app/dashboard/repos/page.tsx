    ...
    return (
        <div className="space-y-8">
            ...
            <div className="p-4 rounded-2xl border-2 border-amber-200 bg-amber-50/80 flex items-start gap-3 shadow-sm">
                <div className="mt-0.5 flex items-center justify-center w-7 h-7 rounded-full bg-amber-100 border border-black">
            ...
            <div className="relative">
                ...
                <input
                    ...
                    className="w-full bg-white border border-black shadow-[0_8px_24px_rgba(15,23,42,0.06)] rounded-xl pl-12 pr-4 py-4 text-gray-900 placeholder-zinc-500 focus:outline-none focus:border-black transition-colors"
                />
            </div>

            {loading ? (
                <div className="flex flex-col items-center justify-center py-20 bg-white border border-black shadow-[0_10px_24px_rgba(15,23,42,0.06)] rounded-3xl">
                    <Loader2 className="w-8 h-8 text-purple-500 animate-spin mb-4" />
                    <p className="text-gray-600">Loading your repositories...</p>
                </div>
            ) : filteredRepos.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 bg-white border border-black shadow-[0_10px_24px_rgba(15,23,42,0.06)] rounded-3xl text-center px-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                    {filteredRepos.map(repo => (
                        <div
                            key={repo.full_name}
                            className="p-6 bg-white border border-black shadow-[0_10px_24px_rgba(15,23,42,0.06)] hover:border-black rounded-2xl transition-all group flex flex-col justify-between"
                        >
                            <div>
                                <div className="flex items-start justify-between mb-3">
                                    <h3 className="font-semibold text-lg text-gray-900 group-hover:text-black transition-colors line-clamp-1 flex-1 pr-4">
                                        {repo.name}
                                    </h3>
                                    {repo.private ? (
