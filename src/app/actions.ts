...
import { getCachedSecurityScanResult, cacheSecurityScanResult, isKvConfigured } from "@/lib/cache";
...
export async function fetchGitHubData(input: string) {
    ...
    if (parsed.kind === "profile") {
        try {
            const data = await getProfile(parsed.username);
            if (session?.user?.id) {
                await recordSearch(session.user.id, parsed.username, "profile");
            }
    ...
    if (parsed.kind === "repo") {
        ...
        try {
            ...
            if (session?.user?.id) {
                await recordSearch(session.user.id, query, "repo");
            }
    ...
    return { error: "Invalid input format. Use owner/repo or a GitHub URL." };
}
...
export async function scanRepositoryVulnerabilities(
    ...
    if (cachedResult) {
        ...
        if (config.analysisProfile === "deep" && limitKey && isKvConfigured()) {
            try {
                ...
                if (error instanceof Error && error.message.includes("Monthly Deep Scan")) {
                    throw error;
                }
                console.warn("Deep-scan quota check skipped (KV unavailable):", error);
            }
        }
    ...
    if (config.analysisProfile === "deep" && limitKey && !isCacheHit && isKvConfigured()) {
        try {
            await kv.incr(limitKey);
            await kv.expire(limitKey, 32 * 24 * 60 * 60);
        } catch (error) {
            console.warn("Deep-scan quota increment skipped (KV unavailable):", error);
        }
    }
...
export async function getRemainingDeepScans(): Promise<{ used: number; total: number; resetsAt: string; isUnlimited: boolean }> {
    ...
    try {
        ...
        console.warn("Deep-scan quota lookup failed; allowing scans locally:", error);
        return { used: 0, total, resetsAt, isUnlimited: false };
    }
}
