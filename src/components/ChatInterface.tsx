...
export function ChatInterface({ repoContext, onToggleSidebar, initialPrompt }: ChatInterfaceProps) {
    ...
    const handleSubmit = async (
        ...
        try {
            ...
            if (isAuthError) {
                ...
                const detail = error instanceof Error ? error.message : "An unexpected error occurred. Please try again.";
    ...
    return (
        <div className="flex flex-col h-full bg-white text-gray-900 relative">
            {/* Repo Header */}
            <div className="sticky top-0 z-20 border-b-2 border-black bg-white shrink-0">
                <div className="flex items-center justify-between px-4 h-16 w-full gap-4">
                    ...
                    <div className="flex items-center gap-3 min-w-0 shrink">
                        ...
                        <div className="flex items-center gap-2 min-w-0 ml-2">
                            ...
                            <Link
                                ...
                                className="hidden lg:flex items-center text-[10px] font-bold tracking-wider uppercase px-2 py-0.5 rounded-md bg-gray-100 text-gray-600 hover:bg-black hover:text-white transition-all border border-black"
                            >
                                Profile
                            </Link>
                        </div>
                    ...
                    <div className="flex items-center gap-3 shrink-0 overflow-x-auto no-scrollbar pr-2">
                        <div className="hidden md:flex items-center p-1 bg-white border border-black rounded-xl gap-1">
                            <button
                                onClick={() => handleSubmit(undefined, ARCHITECTURE_PROMPT)}
                                disabled={loading || scanning}
                                className="flex items-center gap-2 px-3 py-1.5 text-xs font-semibold text-gray-700 hover:bg-black hover:text-white rounded-lg transition-all disabled:opacity-50"
                            >
            ...
            >
                {selectionAnchor && (
                    <button
                        ...
                        className="absolute z-20 -translate-y-full -mt-2 px-3 py-1 bg-white text-gray-900 text-xs rounded-full shadow-lg border border-black transition-transform transition-shadow duration-150 ease-out hover:-translate-y-[110%] hover:scale-105 hover:shadow-xl"
                        style={{ left: selectionAnchor.x, top: selectionAnchor.y }}
                    >
                        Ask <ProductName className="text-xs" />
                    </button>
                )}
                <AnimatePresence initial={false}>
                    {messages.map((msg) => {
                        ...
                        return (
                            ...
                            >
                                {msg.role === "model" && (msg.content || (msg.modelUsed !== "thinking" && loading && isLatestMessage)) && (
                                <div className="h-10 w-10 max-h-10 max-w-10 shrink-0 overflow-hidden">
                                    <BotIcon />
                                </div>
                                )}
                                    ...
                                    {(msg.role === "user" || msg.content || (msg.modelUsed !== "thinking" && loading && msg.id === messages[messages.length - 1]?.id)) && (
                                        <div className={cn(
                                            ...
                                            msg.role === "user"
                                                ...
                                                : "bg-white border border-black text-gray-900 rounded-tl-none"
                                        )}
                                        ...
                                        >
                                            ...
                                            <div className={cn(
                                                "prose prose-sm max-w-none leading-relaxed wrap-break-word overflow-hidden w-full min-w-0",
                                                msg.role === "user"
                                                    ? "text-white prose-p:text-white prose-li:text-white prose-strong:text-white prose-code:text-white"
                                            ...
                                            {msg.scanId && (
                                                <div className="mt-4 pt-4 border-t-2 border-gray-200 flex items-center gap-3">
                                                    ...
                                                    <button
                                                        ...
                                                        className="flex items-center gap-2 px-3 py-1.5 text-xs font-medium text-gray-800 bg-white hover:bg-gray-100 border border-black rounded-lg transition-colors shadow-sm"
                                                    >
                                            ...
                                            {msg.isQuickSecurityScan && (
                                                <div className="mt-4 pt-4 border-t-2 border-gray-200">
                                                    ...
                                                    <button
                                                        ...
                                                        disabled={loading || scanning}
                                                        className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-black hover:bg-neutral-800 border border-black rounded-xl transition-all disabled:opacity-50 group"
                                                    >
            ...
            <div className="p-4 border-t-2 border-gray-200 bg-white/80 backdrop-blur-lg space-y-3">
                {referenceText && (
                    <div className="max-w-4xl mx-auto">
                        <div className="flex items-center gap-2 bg-white border border-black rounded-lg px-3 py-2 text-xs text-gray-700">
                            <span className="text-gray-600 font-medium">Ask <ProductName className="text-xs" /></span>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {REPO_SUGGESTIONS.map((suggestion, index) => (
                                <button
                                    ...
                                    className="text-sm px-4 py-2 bg-white hover:bg-black hover:text-white border border-black hover:border-black rounded-full text-gray-700 transition-all font-medium"
                                >
                                    {suggestion}
                                </button>
                            ))}
