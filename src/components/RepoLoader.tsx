...
export function RepoLoader({ query, initialPrompt }: RepoLoaderProps) {
    ...
    if (error) {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen bg-white text-gray-900 gap-4 p-4">
                <AlertCircle className="w-16 h-16 text-red-500" />
                <h1 className="text-2xl font-bold">Error Loading Repository</h1>
                <p className="text-gray-600 text-center max-w-md">{error}</p>
                <Link href="/" className="mt-4 px-6 py-3 bg-black text-white font-bold border border-black rounded-lg hover:bg-neutral-800 transition-colors shadow-md shadow-slate-900/10">
                    Back to Home
                </Link>
            </div>
        );
                ...
                <div className="max-w-md w-full p-8">
                    <div className="mb-8 text-center">
                        <motion.div
                            ...
                            className="mb-4 inline-flex items-center justify-center w-20 h-20 rounded-2xl bg-white border border-black"
                        >
                    ...
                    <div className="space-y-3">
                        {steps.map((step, index) => (
                            <motion.div
                                ...
                                className="flex items-center gap-3 p-3 bg-white border border-black rounded-lg"
                            >
                                {step.status === "loading" && (
