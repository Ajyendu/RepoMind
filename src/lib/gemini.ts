import { getGenAI, isGoogleSearchEnabled, resolveModelName, withModelFallback, type ModelPreference } from "./ai-client";
...
function supportsThinkingConfig(modelName: string): boolean {
  return /gemini-(2\.5|3)/i.test(modelName);
}

function generationConfigFor(modelName: string, preference: ModelPreference): GenerationConfig | undefined {
  if (preference !== "thinking" || !supportsThinkingConfig(modelName)) {
    return undefined;
  }
  return getThinkingGenerationConfig(true, "HIGH");
}
  ...
  try {
    ...
    const result = await withModelFallback(async (modelName) => {
      const model = getGenAI().getGenerativeModel({
        model: modelName,
        generationConfig: generationConfigFor(modelName, modelPreference),
      });
...
async function pruneFileTreeHierarchically(question: string, fileTree: string[]): Promise<string[]> {
  ...
  try {
    const result = await withModelFallback(async (modelName) => {
      const model = getGenAI().getGenerativeModel({
        model: modelName,
        generationConfig: generationConfigFor(modelName, "flash"),
      });
      return model.generateContent(prompt);
    });
    const response = result.response.text();
...
export async function answerWithContext(
  ...
  } else if (isGoogleSearchEnabled(resolveModelName(modelPreference))) {
  ...
  const result = await withModelFallback(async (modelName) => {
    ...
    let sendResult = await chat.sendMessage(prompt);

    const funcs = sendResult.response.functionCalls?.();
    if (funcs && funcs.length > 0) {
      ...
      sendResult = await chat.sendMessage([{
        functionResponse: {
          name: call.name,
...
export async function* answerWithContextStream(
  ...
  const { chat, firstResult } = await withModelFallback(async (modelName) => {
    const model = getGenAI().getGenerativeModel({
      model: modelName,
      tools: tools.length ? tools : undefined,
      generationConfig: generationConfigFor(modelName, modelPreference),
    ...
    const initialResult = await chatSession.sendMessage(prompt);
    return { chat: chatSession, firstResult: initialResult };
  }, modelPreference);
